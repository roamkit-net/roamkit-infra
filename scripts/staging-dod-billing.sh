#!/usr/bin/env bash
# Phase 3 / PR-5 staging DoD: billing env → deposit-info → verify → ledger → balance → order.
#
# No feature code — ops verification against a live staging stack.
#
...
#   SKIP_ORDER=1 ./scripts/staging-dod-billing.sh          # stop after ledger/balance
#   VERIFY_TX_HASH=0x... ./scripts/staging-dod-billing.sh  # real CEX verify path
#
# Env:
...
DOD_EMAIL="${DOD_EMAIL:-billing-dod-$(date +%s)@example.com}"
...
fail() {
  echo "STAGING BILLING DoD FAILED: $*" >&2
  exit 1
}
...
echo "Phase 3 staging billing DoD against ${API_URL}"
...
entry = credit_service.admin_adjust(
    ...
    credit=True,
    reason='staging-dod-billing deposit stand-in',
    actor_id='staging-dod-billing',
    idempotency_key='staging-dod-credit-${DOD_EMAIL}',
)
...
if need > 0:
    credit_service.admin_adjust(
        ...
        need,
        credit=True,
        reason='staging-dod-billing top-up for order',
        actor_id='staging-dod-billing',
        idempotency_key='staging-dod-topup-${DOD_EMAIL}',
    )
print('funded')
"
...
if [[ "${SKIP_ORDER}" == "1" ]]; then
  ...
  echo "STAGING BILLING DoD PASSED (ledger/balance only)"
  exit 0
fi
...
echo "STAGING BILLING DoD PASSED"
